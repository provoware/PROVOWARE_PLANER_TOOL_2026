...
REQUIRED_FILES = {
    ".gitignore", "NUTZERANLEITUNG.md", "contracts/TRANSPORT_PROFILE_CONTRACT.json",
    "standards/TRANSPORT_STANDARD.json", "tools/transport_profiles.py",
    "tools/package_transport.py", "tests/test_i014_transport_profiles.py",
    "tests/test_i014_runtime_package.py", "docs/I014_TRANSPORTPROFILE_TRENNUNG.md",
...
def validate() -> dict:
    ...
    errors.extend(f"I014_DATEI_FEHLT: {path}" for path in sorted(REQUIRED_FILES - files))
    ...
    for required in ("VERSION.json", "requirements-gui.lock", "contracts/GUI_RUNTIME_CONTRACT.json", "tools/start_gui.py", "NUTZERANLEITUNG.md"):
        if required not in user:
            errors.append(f"I014_NUTZER_RUNTIME_FEHLT: {required}")
    for path in sorted(user):
        if path.startswith((".github/", "docs/", "standards/", "tests/", "tools/autopilot/")):
    ...
    builder = (ROOT / "tools/package_transport.py").read_text(encoding="utf-8")
    ...
    # Der aktuelle ITERATION_PLAN gehört ab I015 der Folgeiteration. Historische
    # I014-Planidentität wird nur geprüft, solange I014 selbst der aktuelle Stand ist.
    criteria_count = 10
    if current == 14:
    ...
    workflow = (ROOT / ".github/workflows/i014-qualifikation.yml").read_text(encoding="utf-8")
    for token in ("preflight.py", "candidate_inventory.py --write-manifest", "test_i014_transport_profiles.py", "test_i014_runtime_package.py", "package_transport.py --profile NUTZER", "--offscreen-smoke", "verify_only", "verify_sha"):
        if token not in workflow:
            errors.append(f"I014_WORKFLOW_GATE_FEHLT: {token}")

    return {
        "status": "PASS" if not errors else "FAIL", "errors": errors,
        "repository_files": len(files), "classified_paths": profile_validation.get("classified_paths", 0),
        "user_files": len(user), "developer_files": len(developer), "evidence_files": len(evidence),
        "acceptance_criteria": criteria_count, "runtime_schema_unchanged": 4,
    }
...
def main() -> int:
    ...
    print(f"Profile: Nutzer={result['user_files']} Entwickler={result['developer_files']} Evidence={result['evidence_files']} | klassifiziert={result['classified_paths']}")
