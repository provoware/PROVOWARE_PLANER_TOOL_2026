...
def validate() -> dict:
    ...
    if ui_standard.get("color_alone_forbidden") is not True: errors.append("I005_FARBALLEIN_STANDARD_FEHLT")
    if access_standard.get("rules", {}).get("accessible_names_required") is not True: errors.append("I005_ACCESSIBLE_NAMES_STANDARD_FEHLT")
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="provoware-i005-validator-") as temp:
            workspace = Path(temp); service = open_calendar_service(workspace / "planer.sqlite3")
            window = CalendarWindow(service, repo_root=ROOT, workspace=workspace)
            try:
                window.resize(1280, 720); window.show(); app.processEvents()
                for mode in CalendarViewMode: window._set_mode(mode); app.processEvents()
                if len(window.marker_labels) != 5: errors.append("I005_RUNTIME_MARKER_ANZAHL_FALSCH")
                if not all(label.text().strip() for label in window.marker_labels): errors.append("I005_RUNTIME_MARKER_TEXT_FEHLT")
                if window.stack.count() != 4: errors.append("I005_RUNTIME_VIEW_ANZAHL_FALSCH")
                runtime_result = "PASS"
            finally:
