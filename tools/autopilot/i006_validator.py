...
def validate() -> dict:
    ...
    if current < 6: errors.append("I006_VERSION_UNTER_MINDESTSTAND")
    if current == 6 and version.get("version") != "0.6.0-dev.1": errors.append("I006_VERSION_FALSCH")
    if _iteration(status.get("iteration")) < 6: errors.append("I006_STATUS_UNTER_MINDESTSTAND")
    if _iteration(project.get("foundation", {}).get("current_iteration")) < 6: errors.append("I006_PROJEKTVERTRAG_UNTER_MINDESTSTAND")
    if current > 6 and "I006-TODO-DOMAIN-LINK" not in status.get("last_completed", []): errors.append("I006_HISTORISCHER_CHECKPOINT_FEHLT")
