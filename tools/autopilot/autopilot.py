...
def main() -> int:
    ...
    if current >= 2:
        gates.append(("I002-EVIDENCE-GATE", validate_i002()))
    if current >= 3:
    ...
    if current >= 6:
        gates.append(("I006-TODO-DOMAIN-LINK-GATE", validate_i006()))
    if current >= 7:
        gates.append(("I007-TODO-GUI-VIEWMODEL-GATE", validate_i007()))
    if current >= 8:
        gates.append(("I008-SYNC-KONFLIKT-GATE", validate_i008()))
    if current >= 9:
        gates.append(("I009-FELD-BASELINE-TRANSAKTIONS-GATE", validate_i009()))
    if current >= 10:
    ...
    if args.command == "qualifizieren":
        print(
            ...
            "Pflichtgates bis zur aktuellen Iteration sind konsistent."
        )
    return 0
