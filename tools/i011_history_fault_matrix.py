...
FAULT = "SYNC_AFTER_HISTORY_SNAPSHOT"


def _seed(database_path: Path) -> tuple[str, str]:
    ...
    services.todos.update_todo(replace(todo, title="Neue Aufgabe"), expected_version=todo.version)
    return link.link_id, event.event_id


def _verify_rolled_back(database_path: Path, link_id: str, event_id: str) -> bool:
    services = open_planner_services(database_path)
    event = services.calendar.get_event(event_id)
    plan = services.sync.plan(link_id)
    services.database.quick_check()
    return (
        event.title == "Basis"
        and services.sync.receipt_count(link_id) == 0
        and services.journal.history_repository.snapshot_count(link_id) == 0
        and plan.write_permitted
    )


def _exception_scenario() -> dict:
    with tempfile.TemporaryDirectory(prefix="provoware-i011-history-fault-") as temp:
        database_path = Path(temp) / "planer.sqlite3"
        link_id, event_id = _seed(database_path)
        services = open_planner_services(database_path)
        plan = services.sync.plan(link_id)
        ...
        os.environ["PROVOWARE_SYNC_FAULT_MODE"] = FAULT
        caught = False
        try:
            services.sync.commit(plan)
        ...
        return {
            "scenario": FAULT,
            "pass": caught and _verify_rolled_back(database_path, link_id, event_id),
        }


def _child(database_path: Path, link_id: str) -> int:
    services = open_planner_services(database_path)
    services.sync.commit(services.sync.plan(link_id))
    return 0


def _crash_scenario() -> dict:
    with tempfile.TemporaryDirectory(prefix="provoware-i011-history-crash-") as temp:
        database_path = Path(temp) / "planer.sqlite3"
        link_id, event_id = _seed(database_path)
        env = os.environ.copy()
        env["PROVOWARE_SYNC_FAULTS"] = "1"
        env["PROVOWARE_SYNC_CRASH_MODE"] = FAULT
        ...
        return {
            "scenario": "PROCESS_EXIT_AFTER_HISTORY_SNAPSHOT",
            "returncode": result.returncode,
            "pass": result.returncode == 92
            and _verify_rolled_back(database_path, link_id, event_id),
        }


def run_matrix() -> dict:
    scenarios = [_exception_scenario(), _crash_scenario()]
    return {
        "status": "PASS" if all(item["pass"] for item in scenarios) else "FAIL",
        "scenarios": scenarios,
    }
