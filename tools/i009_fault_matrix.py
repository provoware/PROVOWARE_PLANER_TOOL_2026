...
def _verify_rolled_back(database_path: Path, link_id: str, event_id: str) -> bool:
    ...
    baselines = {item.field_id: item.baseline_sha256 for item in services.sync.baselines(link_id)}
...
def _exception_scenario(name: str) -> dict:
    with tempfile.TemporaryDirectory(prefix=f"provoware-i009-{name.lower()}-") as temp:
        ...
        return {"scenario": name, "pass": caught and _verify_rolled_back(database_path, link_id, event_id)}
...
def _crash_scenario() -> dict:
    with tempfile.TemporaryDirectory(prefix="provoware-i009-process-crash-") as temp:
        database_path = Path(temp) / "planer.sqlite3"
        link_id, event_id = _seed(database_path)
        env = os.environ.copy()
        ...
        return {
            "scenario": "PROCESS_EXIT_AFTER_ENTITY_WRITE",
            "returncode": result.returncode,
            "pass": result.returncode == 92 and _verify_rolled_back(database_path, link_id, event_id),
        }
